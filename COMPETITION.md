...
| Handles the declared administrative policy forms | Locked held-out outcomes, coverage and failure counts | Partial: 6 typed families implemented and tested (21/21 starter cases, `bench/cases.py` + `bench/run_benchmark.py`), now including a real completeness check (`agent/gate/completeness.py`) that flags unclassified number+unit claims (e.g. "8-12 hours") instead of silently defaulting to SUPPORTED — closed 2026-09-13 after being found live via a real Groq reply. 200-case locked holdout suite not built |
...
- [x] Required sponsor integration actually runs in the deployed workflow. **Confirmed live at https://hesitate-v2.onrender.com/gate/live-demo** — queries the real Moss project, not a local fixture. Recreated the deployed service once more with the full env var set (Moss, LiveKit, Deepgram, Groq) since Render's CLI only sets env vars at creation time; deleted both prior services (`hesitate`, `hesitate-api`) once this final one was verified, ending on the clean name `hesitate.onrender.com`.
